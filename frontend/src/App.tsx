...
import { AppThemeContext, AppTheme } from './themeContext'
...
function App() {
  // 全局主题（明/暗）——单一数据源，持久化到 localStorage
  const [appTheme, setAppTheme] = useState<AppTheme>(
    () => (localStorage.getItem('appTheme') as AppTheme) || 'light'
  );
  useEffect(() => {
  ...
  const appThemeCtx = {
    theme: appTheme,
    toggle: () => setAppTheme(p => (p === 'light' ? 'dark' : 'light')),
    setTheme: (t: AppTheme) => setAppTheme(t),
  };

  return (
    <QueryClientProvider client={queryClient}>
      <AppThemeContext.Provider value={appThemeCtx}>
        <ConfigProvider
          theme={{
