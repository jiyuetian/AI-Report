...
export default function DashboardOps({ id, title, onRename, onDelete }: DashboardOpsProps) {
  ...
  const menuItems = {
    items: [
      { key: 'edit', icon: <EditOutlined />, label: '编辑看板' },
      { type: 'divider' as const },
      { key: 'share', icon: <ShareAltOutlined />, label: '分享看板' },
      { key: 'export', icon: <ExportOutlined />, label: '导出看板' },
      { key: 'version', icon: <HistoryOutlined />, label: '版本管理' },
      { key: 'lineage', icon: <ApartmentOutlined />, label: '查看血缘' },
    ...
    onClick: ({ key }: { key: string }) => {
      ...
      else if (key === 'version') setVersionOpen(true)
      else if (key === 'lineage') nav(`/lineage?dashboard_id=${id || ''}&from=/dashboard`)
