...
import { InboxOutlined, FileExcelOutlined, CloseCircleOutlined, ReloadOutlined, DownloadOutlined, FileTextOutlined, ExclamationCircleOutlined, CheckCircleOutlined, WarningOutlined } from '@ant-design/icons'
...
export default function UploadPage() {
  ...
  const uploadFile = async (file: File, uid: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        // 处理错误
        const errorCode = data.detail?.code || 'UPLOAD_500'
        const errorMessage = data.detail?.message || '上传失败'
