...
class FileParser:
    ...
    def read_excel_sheet(
        ...
        """读取指定Sheet。header_row 指定表头所在行（0基），None 表示用首行。"""
        kwargs: Dict[Any, Any] = {}
        ...
        if sheet_name:
            ...
            return pd.read_excel(file_path, sheet_name=sheet_index, **kwargs)
        else:
            return pd.read_excel(file_path, **kwargs)  # 默认第一个sheet
    
    # ==================== P1-2: 表头探测打分（多行标题表识别） ====================
    # 多行说明型表格（首行是合并标题/说明，真正列头在下方几行）按"首行即表头"解析会
    # 把说明文字当列名。这里对前 N 行打分，选最可能的一行作为表头，降低误判。
    MAX_HEADER_SCAN = 6
    # 说明/标题行常见关键词（命中则强烈偏向"这不是列头"）
    _DESC_KEYWORDS = ("说明", "注：", "注:", "备注", "单位：", "单位:", "制表", "合计", "共", "页", "日期：", "日期:")
    ...
    @classmethod
    def _score_header_row(cls, row_cells: List[Any]) -> float:
        """对一行打分，分数越高越像真正的列头行。"""
        n = len(row_cells)
        if n == 0:
            return -1e9
        ...
        # 标题/说明行常只填首格（合并单元格），fill 低 → 扣分；列头行应填满多数列
        score = fill_ratio * 2.0
        # 说明关键词命中扣分
        kw_hits = sum(1 for c in non_empty if any(k in str(c) for k in cls._DESC_KEYWORDS))
        score -= 1.5 * kw_hits
        # 平均单元格长度：列头短、说明长
        avg_len = sum(len(str(c).strip()) for c in non_empty) / len(non_empty)
        if avg_len > 15:
            score -= (avg_len - 15) * 0.1
        # 含超长单元格（>30 字符，典型的说明句）直接视为说明行
        if any(len(str(c).strip()) > 30 for c in non_empty):
            score -= 2.0
        # 列头多为非数值串；出现数值单元格略扣分（更像数据行）
        numeric = sum(1 for c in non_empty if cls._looks_numeric(c))
        score -= 0.3 * numeric
    ...
    def _detect_header_row(
        ...
        sheet_name: Optional[str] = None,
        sheet_index: Optional[int] = None,
        n_scan: int = MAX_HEADER_SCAN
    ) -> int:
        """扫描前 n_scan 行，返回最像列头的行索引（0 基）。找不到时回退 0。"""
        target = sheet_name if sheet_name else (0 if sheet_index is None else sheet_index)
        try:
            raw = pd.read_excel(file_path, sheet_name=target, header=None, nrows=n_scan)
        except Exception as e:
            print(f"[FileParser] 表头探测失败(回退首行): {e}")
            return 0
        best_idx, best_score = 0, -1e9
        for i in range(min(n_scan, len(raw))):
            cells = raw.iloc[i].tolist()
            s = cls._score_header_row(cells)
            if s > best_score:
    ...
    def parse_file(
        ...
        if file_ext in ['.xlsx', '.xls']:
            ...
            # 如果指定了sheet，读取数据
            if sheet_name:
                # P1-2 表头探测：多行标题表自动定位真正列头行
                hdr = cls._detect_header_row(file_path, sheet_name=sheet_name)
                df = cls.read_excel_sheet(file_path, sheet_name=sheet_name, header_row=hdr)
                ...
                # 只有一个sheet，自动读取（P1-2 表头探测）
                hdr = cls._detect_header_row(file_path, sheet_index=0)
                df = cls.read_excel_sheet(file_path, sheet_index=0, header_row=hdr)
                df = cls._process_sensitive_data(df)  # M1-11脱敏
                result["preview"] = cls.preview_data(df)
                result["dataframe"] = df
                result["header_row_index"] = hdr
