...
class S3LLMEnhancer:
    ...
    def __init__(self):
        ...
        # 已验证派生指标（由 derived_metric_service 反推），S3 阶段注入
    ...
    def _apply_derived_metrics(
        ...
        """
        派生指标反哺（LLM 输出的兜底校正）：
          - 指标列是已验证派生指标 → 把加工公式写进 config.derived_metric（血缘/前端白盒展示）
          - 比率类指标若被 LLM 写成 sum/求和 → 强制纠正为 avg（比率求和无业务意义）
        LLM 只被"告知"，是否照做不可控，所以必须在代码侧硬保证。
        """
        if not derived_metrics or not charts:
            return charts
        try:
            from app.core.derived_metric_service import recommended_agg, is_ratio_metric
        except Exception:  # 服务不可用时静默跳过，不影响看图
            return charts

        for ch in charts:
            if not isinstance(ch, dict):
                continue
            metric = ch.get("y_field") or ch.get("value_field")
            if not metric or metric not in derived_metrics:
                continue
            ...
            if not isinstance(cfg, dict):
                cfg = {}
                ch["config"] = cfg
            # 纠正求和：顶层 aggregate 与 config 内聚合口径都要管
            if ratio:
                top = str(ch.get("aggregate") or ch.get("aggregation") or "").lower()
                if top in ("sum", "count"):
                    ch["aggregate"] = "avg"
                    ch.pop("aggregation", None)
                inner = str(cfg.get("aggregate") or cfg.get("aggregation") or "").lower()
                if inner in ("sum", "count"):
                    cfg["aggregate"] = "avg"
                    cfg["aggregation"] = "avg"
            cfg.setdefault("aggregation", agg)
            cfg["derived_metric"] = {
                "metric": metric,
                ...
                "aggregation": cfg.get("aggregation", agg),
                "ratio": ratio,
            }
        return charts
...
"""
        ...
        # 让 LLM 知道「抵押率」是算出来的比率，而不是可以随便求和的普通数值列。
        derived_metrics = derived_metrics or {}
        if derived_metrics:
            try:
                from app.core.derived_metric_service import annotate_fields
                fields_desc = annotate_fields(fields_desc, derived_metrics)
            except Exception:
                pass
        derived_rule = (
            "8. 字段描述中标注了「派生指标：X = A ÷ B」的列是算出来的比率/差额指标，"
            "只能按 avg（平均值）汇总，严禁使用 sum/求和（比率求和无业务意义）；"
            "且不要把它和它的分量列（A 或 B）放进同一张图重复表达"
...
请只输出JSON，不要其他文字。"""
    ...
    async def generate_with_self_healing(
        ...
        if derived_metrics is not None:
            ...
            self.rule_engine = S3ChartEngine(derived_metrics=derived_metrics)
        ...
        for attempt in range(self.MAX_RETRIES + 1):
            ...
            response = await llm_chat(
                ...
                timeout=60,  # 图表推荐输出较长；超时后由规则引擎兜底出图
            )
...
输出格式：与输入相同的JSON数组"""
        ...
        response = await llm_chat(
            ...
            json_mode=True,
            user_id="s3_llm_enhancer",
            timeout=45,  # 字段语义增强（非关键路径），超时直接放弃增强
        )
