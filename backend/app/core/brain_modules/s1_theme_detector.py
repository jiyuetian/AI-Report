...
class S1ThemeDetector:
    ...
    async def _llm_fallback(
        ...
        response = await llm_chat(
            ...
            timeout=25,  # 主题识别只回一个短 JSON，25s 足够；超时走词典兜底
        )
        
        if response.success and response.response_json:
            result = response.response_json
            return ThemeDetectionResult(
                theme_tag=result.get("theme_tag", "数据概览"),
    ...
    async def detect(
        ...
        # 4. LLM兜底（use_llm=False 时跳过，支持 LLM 不可达短路降级，避免跑满超时）
        if not use_llm:
            print("[S1] use_llm=False：跳过 LLM 兜底，直接返回词典结果（LLM 不可达降级）")
            return dict_result or ThemeDetectionResult(
                theme_tag="通用分析", confidence=0.0, matched_keywords=[],
                method="dictionary", llm_called=False, reason="LLM 不可达，未调用 LLM"
            )
...
async def detect_theme(
    ...
    result = await detector.detect(db, fields, sample_data, dataset_name, use_llm=use_llm)
