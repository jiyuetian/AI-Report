...
class S3ChartEngine:
    def __init__(self, rules_file: Optional[str] = None,
                 derived_metrics: Optional[Dict[str, Dict[str, Any]]] = None):
        ...
        # 2026-09-17：派生指标（如 抵押率 = 贷款金额 ÷ 抵押物评估价值）反哺给图表引擎。
        # 由 derived_metric_service 用真实数据反推得到，带公式与匹配率，供：
        #   ① 指标打分提权（派生业务指标优先当 y 轴）
        #   ② 比率类强制 avg 聚合（抵押率求和无业务意义）
        #   ③ 图表 config 落公式，血缘/前端可白盒展示加工口径
        self.derived_metrics: Dict[str, Dict[str, Any]] = derived_metrics or {}
        self._load_rules()
    ...
    def _match_rule(
        ...
        used_metrics: Optional[Dict[str, int]] = None
        ...
        # 派生指标优先，但已用过的指标要降权——否则会出现 6 张图全画同一个指标，
        # 丢失多维度覆盖（第一张给派生指标，后续轮换到金额/笔数等其他指标）
        _used = used_metrics or {}
        _derived = self.derived_metrics

        def _score(f: str) -> int:
            # 同一指标最多出现 2 次：既保证派生指标有存在感，又不至于 6 张图全画它
            return _metric_score(f, _derived) - (10 if _used.get(f, 0) >= 2 else 0)

        best = (sorted(number_fields, key=lambda f: (-_score(f),))[0]
                if number_fields else None)
        n0 = best if best is not None else (number_fields[0] if number_fields else None)
        # 次指标不重复选派生指标的分量列（如 y=抵押率 时 n1 不该再是贷款金额），避免同图自证
        _excluded = set()
        if n0 and n0 in _derived:
            _excluded = set(_derived[n0].get("components") or [])
        _n1_pool = [f for f in number_fields if f != n0 and f not in _excluded] or \
                   [f for f in number_fields if f != n0]
        n1 = (sorted(_n1_pool, key=lambda f: (-_metric_score(f, _derived),))[0]
              if _n1_pool else n0)
        def _pick_metric(pool: List[str]) -> Optional[str]:
            pool = [f for f in (pool or []) if f]
            return sorted(pool, key=lambda f: (-_score(f),))[0] if pool else None
        ...
        x = y = cat = val = None
        if ctype in ("line",):
            x, y = d0, _pick_metric(number_fields)
        elif ctype in ("bar",):
            ...
            cat, val = g0, _pick_metric(number_fields)
        elif ctype in ("kpi",):
            y = _pick_metric(number_fields)
        ...
        config.update(self._derived_metric_config(metric_field))

        # 比率类指标（抵押率/逾期率等）不能按金额口径展示：
        #   「总抵押率」语义错误 → 改「平均抵押率」；金额类 KPI 的 ¥ 前缀也要去掉
        if self._is_ratio_field(metric_field):
            if ctype == "kpi":
                if title.startswith("总") and metric_field and metric_field in title:
                    title = "平均" + title[len("总"):]
        ...
        return ChartRecommendation(
            ...
            reason=f"匹配规则(基数/粒度已校验): {rule.get('name', '')}"
                   f"{self._derived_reason(metric_field)}",
        )
    ...
    def _best_metric(
        ...
        derived: Optional[Dict[str, Dict[str, Any]]] = None
    ) -> Optional[str]:
        """指标优先选已验证派生指标，其次金额/率/笔数类数值字段（移植 B 的语义角色打分）"""
        if not number_fields:
            return None
        scored = sorted(number_fields, key=lambda f: (-_metric_score(f, derived),))
        return scored[0]
    ...
    def _is_ratio_field(self, field: Optional[str]) -> bool:
        """比率类字段：已验证派生比率指标，或列名命中率/占比语义"""
        if not field:
            return False
        info = self.derived_metrics.get(field)
        if info:
            try:
                from app.core.derived_metric_service import is_ratio_metric
                return is_ratio_metric(info)
            except Exception:
                return True
        return _metric_role(field) == "rate"

    def _derived_metric_config(self, metric_field: Optional[str]) -> Dict[str, Any]:
        """
        指标列若是「已验证派生指标」，返回要合并进图表 config 的补丁：
          - derived_metric：加工公式/分量列/匹配率（血缘与前端据此白盒展示）
          - aggregation：安全汇总方式（比率类强制 avg，禁止求和）
        非派生指标返回空补丁。
        """
        if not metric_field or metric_field not in self.derived_metrics:
            return {}
        info = self.derived_metrics[metric_field]
        try:
            from app.core.derived_metric_service import recommended_agg, is_ratio_metric
            agg = recommended_agg(info)
            ratio = is_ratio_metric(info)
        except Exception:
            agg, ratio = "avg", True
        return {
            "aggregation": agg,
    ...
    def _derived_reason(self, metric_field: Optional[str]) -> str:
        """派生指标的推荐理由后缀（让用户/血缘知道这个指标是怎么算出来的）"""
        if not metric_field or metric_field not in self.derived_metrics:
            return ""
        info = self.derived_metrics[metric_field]
        return f"；指标「{metric_field}」为派生指标：{info.get('expression', '')}"
    ...
    def recommend_charts(
        ...
        # 派生指标可在构造时或调用时传入（调用时优先），供指标打分与聚合约束使用
        ...
        if self.derived_metrics:
            derived_brief = ", ".join(
                f"{k} = {v.get('expression', '')}" for k, v in self.derived_metrics.items()
            )
            print("[S3-v2] 派生指标(已反哺): " + derived_brief)
        ...
        for rule in sorted(self.rules, key=lambda x: x.get("priority", 0), reverse=True):
            rec = self._match_rule(rule, field_types, grain, cardinality, used_metrics)
            if not rec:
                continue
            ...
            if any(r.chart_type == rec.chart_type and self._same_fields(r, rec) for r in recs):
                continue
            recs.append(rec)
            # 记录已用指标，让后续规则轮换到其他指标（避免多图同一个指标）
            _m = rec.y_field or rec.value_field
            if _m:
                used_metrics[_m] = used_metrics.get(_m, 0) + 1
            if len(recs) >= max_charts:
                break

        if len(recs) < max_charts:
            recs = self._apply_fallback(recs, field_types, grain, cardinality, max_charts, used_metrics)
    ...
    def _apply_fallback(
        ...
        for fb in self.fallback.get("default_charts", []):
            ...
            # 兜底图同样轮换指标，避免与主规则图重复
            _pool = [f for f in num_f if _used.get(f, 0) == 0] or num_f
            best_metric = self._best_metric(_pool, field_types, self.derived_metrics)
            _used[best_metric] = _used.get(best_metric, 0) + 1 if best_metric else 0
            rec = ChartRecommendation(
                ...
                config=self._derived_metric_config(best_metric),
            )
            result.append(rec)
    ...
    def generate_dashboard_config(
        ...
        return {
            ...
            "llm_used": False,
            # 反哺结果一并返回，便于落库与前端展示「本看板用到了哪些派生指标」
...
def _metric_score(field: str, derived: Optional[Dict[str, Dict[str, Any]]] = None) -> int:
    """指标优先级打分：已验证派生指标 > 金额 > 率 > 笔数 > 年龄/时长 > 其他数值

    派生指标（如 抵押率 = 贷款金额 ÷ 抵押物评估价值）是业务核心口径，
    比单纯的金额列更能反映经营质量，故置于最高优先级。
    """
    if derived and field in derived:
        return 4
...
def recommend_charts(fields, grain="detail", sample_data=None, derived_metrics=None):
    return [r.to_dict() for r in S3ChartEngine(
        derived_metrics=derived_metrics
    ).recommend_charts(fields, grain, sample_data)]


def generate_dashboard(fields, grain="detail", sample_data=None, derived_metrics=None):
    return S3ChartEngine(
        derived_metrics=derived_metrics
    ).generate_dashboard_config(fields, grain, sample_data)
