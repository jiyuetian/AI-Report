...
async def logout(request: Request, current_user = Depends(get_current_user)):
    """用户登出（D2-5：吊销当前 token，旧 token 立即失效）"""
    jti = current_user.get("jti")
    exp = current_user.get("token_exp")
    if jti:
        from datetime import datetime as _dt
        exp_dt = _dt.utcfromtimestamp(exp) if isinstance(exp, (int, float)) else _dt.utcnow()
...
async def register(request: Request, reg: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    安全说明：本端点对所有人开放创建普通用户（is_superuser=False），用于补齐"无自注册入口"缺陷。
    生产环境若仅需内部使用，应将其关闭或改为邀请制/管理员创建。
    """
    # 用户名/邮箱唯一性校验
    existing = (await db.execute(
        select(User).where((User.username == reg.username) | (User.email == reg.email))
    )).scalars().first()
    if existing:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail={"code": "USER_EXISTS", "message": "用户名或邮箱已存在"}
        )
    ...
    return {"success": True, "message": "注册成功", "user_id": str(new_user.id)}