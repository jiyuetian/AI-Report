...
:: ---------- 1. 释放端口（仅前端；后端单实例由 run_backend.py 的 B5 守卫保证，不在此强杀，避免双进程抢 DuckDB） ----------
echo [1/4] 释放前端端口...
for /f "tokens=5" %%p in ('netstat -ano ^| findstr ":5173 " ^| findstr LISTENING') do taskkill /PID %%p /F >nul 2>&1

:: ---------- 2. 启动后端（统一走 run_backend.py，自带 B5 单实例守卫：pidfile+端口双检，重复点击会被拒） ----------
echo [2/4] 启动后端服务(自动用SQLite，无需装数据库)...
start "AIBI-Backend" /min cmd /c ""%PYTHON%" "%ROOT%backend\run_backend.py" > "%ROOT%backend\server.log" 2>&1"
set /a tries=0
:wait_backend
