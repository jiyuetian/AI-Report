...
18. **2.4 前端 AI 展示徽标落地（✅ 完成）** — 看板标题旁绿标「AI 参与生成」/灰标「规则兜底生成」（`DashboardOps.tsx`，措辞对齐用户）+ 目标区 `s2_generated_by` 小标「目标·AI / 目标·规则」；`DashboardPage.tsx` `DashboardConfig` 加 `s2_generated_by` 类型、`s2Gen` 推导、`s2GeneratedBy` 传参；tsc EXIT=0。演示数据 `backend/scripts/make_demo_db.py`：克隆 `qa_aibi.db`→`demo_aibi.db`（不动线上库），改写两看板 config（A=ai/llm 绿标、B=rule 灰标），duckdb dict→JSON 机制已验证。本机停后端后运行即得 `demo_aibi.db` 供路演开两看板验证徽标。方案 `plan_31_upload_layout.md`（3.1，待确认）。
